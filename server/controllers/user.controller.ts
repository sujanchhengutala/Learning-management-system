...
export const activateUser = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (existUser) {
            return next(new ErrorHandler("email id is already exist", 400))
        }
...
export const logoutUser = async (req: Request | any, res: Response, next: NextFunction) => {
    console.log("logout")
    try {
...
//since access token expire in 5 minute update access token

export const updateAccessToken = async (req: Request | any, res: Response, next: NextFunction) => {
    try {
        ...
        if (!decode) {
            return next(new ErrorHandler("couldnot refresh token", 400))
        }
...
export const updatePassword = async(req:Request|any, res:Response, next:NextFunction)=>{
    try {
        ...
        if(oldPassword===newPassword){
            return next(new ErrorHandler("new password is simillar to old password", 400))
        }
...
export const updateProfilePicture = async(req:Request|any, res:Response, next:NextFunction)=>{
    ...
    try {
        ...
        if(avatar && user){
            if(user?.avatar?.public_id){
                //if avatar is already present first delete it and upload
            ...
            //if there is no avatar then directly upload
            else{
                const myCloud = await cloudinary.v2.uploader.upload(avatar,{
                    folder:"avatars",
                    width:150
